/**
 * UserMenu.jsx
 * Shows the signed-in user's avatar in the header.
 * Clicking it opens a small dropdown with their name, email,
...
export default function UserMenu() {
  ...
  // Fallback initials avatar if Google picture fails to load
    ...
    <div className="relative" ref={menuRef}>
      ...
      >
        {user.avatar ? (
          ...
          <div className="h-8 w-8 rounded-full bg-accent-blue flex items-center justify-center text-white text-xs font-bold">
            {initials}
          </div>
        )}
      </button>
